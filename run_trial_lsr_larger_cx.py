...
with open('./base/largernet_vb_lsr_cx.py') as oldver, open('./compiled/largernet_vb_lsr_cx_'+conn_weight+'.py','w') as newver: 
	for line in oldver:	
		newver.write(line)        

with open('./param/'+conn_weight+'.txt') as inf: # ./param/ contains all the .txt files with different parameter settings
    for line in inf:
        line_prepender('./compiled/largernet_vb_lsr_cx_'+conn_weight+'.py', line) # put the new file (file+parameters) into a folder called 
        															  # ./compiled, with the name of the file after the parameter
        															  # set's name, then .py as the format

with open('./compiled/largernet_vb_lsr_cx_'+conn_weight+'.py') as oldver, open('./largernet_vb_lsr_cx_'+conn_weight+'.py','w') as newver:
    for line in oldver:
        newver.write(line) 
##############################################################################################
###### creating directories
for iii in range (3,7):  # two light spot conditions
	addpath='./run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(iii)
	if not os.path.exists(addpath): 
		os.makedirs(addpath)
	
	save_relay = './run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(iii)+'/relay'
	if not os.path.exists(save_relay): 
		os.makedirs(save_relay)
	for i in range (0, num_tc_cell):
		save_tc_cell = './run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(iii)+'/relay/tc_cell'+str(i)
		if not os.path.exists(save_tc_cell): 
			os.makedirs(save_tc_cell)
	
	save_reticular = './run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(iii)+'/reticular'
	if not os.path.exists(save_reticular): 
		os.makedirs(save_reticular)
	for i in range (0, num_re_cell):
		save_re_cell = './run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(iii)+'/reticular/re_cell'+str(i)
		if not os.path.exists(save_re_cell): 
			os.makedirs(save_re_cell)

tc_results_dat='/Users/jiemei/Desktop/TC_model/run_lgn_pgn/largernet_cx/'+conn_weight+'/tc_lsr_results_dat/'
if not os.path.exists(tc_results_dat): 
	os.makedirs(tc_results_dat)	

re_results_dat='/Users/jiemei/Desktop/TC_model/run_lgn_pgn/largernet_cx/'+conn_weight+'/re_lsr_results_dat/'
if not os.path.exists(re_results_dat): 
	os.makedirs(re_results_dat)			

rasterplots='./run_lgn_pgn/largernet_cx/'+conn_weight+'/raster_plot/'
...
for ls_condition in range(3,7):
	for trial_num in range(1,101):	
		...
		dir_tc = './run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(ls_condition)+'/relay/tc_cell' # cell 0...n
		dir_re = './run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(ls_condition)+'/reticular/re_cell'
		dir_raster = './run_lgn_pgn/largernet_cx/'+conn_weight+'/raster_plot'
		...
		with open('/Users/jiemei/Desktop/TC_model/param/hoc_name.txt') as ref: # add parameters from the complete parameter file to the python script to run, then run it
			for lines in ref:
				print lines
				line_prepender('largernet_vb_lsr_cx_'+conn_weight+'.py', lines)
		
		os.system('python '+ 'largernet_vb_lsr_cx_'+conn_weight+'.py')

		os.remove('largernet_vb_lsr_cx_'+conn_weight+'.py')
...
for ii in range(3,7):
	for i in range(0, num_tc_cell):
		tc_results = '/run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(ii)+'/relay/tc_cell'
...
for ii in range(3,7):
	for i in range(0, num_re_cell):
		re_results = '/run_lgn_pgn/largernet_cx/'+conn_weight+'/light_spot_'+str(ii)+'/reticular/re_cell'
...
for ii in range(3,7):
	...
	for i in range(0, num_tc_cell):
		os.chdir('/Users/jiemei/Desktop/TC_model')
		tc_results = './run_lgn_pgn/largernet_cx/'+conn_weight+'/tc_lsr_results'
		...
		os.chdir('/Users/jiemei/Desktop/TC_model/run_lgn_pgn/largernet_cx/'+conn_weight+'/tc_lsr_results')
...
for ii in range(3,7):
	...
	for i in range(0, num_re_cell):
		os.chdir('/Users/jiemei/Desktop/TC_model')
		re_results = './run_lgn_pgn/largernet_cx/'+conn_weight+'/re_lsr_results'
		...
		plt.plot(re_plot)
		#plt.ylim(0, 30)
		os.chdir('/Users/jiemei/Desktop/TC_model/run_lgn_pgn/largernet_cx/'+conn_weight+'/re_lsr_results')
