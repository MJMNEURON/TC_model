# runs the contrast stimuli with smallnet
...
with open('./base/smallnet_vb_contrast.py') as oldver, open('./compiled/smallnet_vb_contrast_'+conn_weight+'.py','w') as newver: 
	for line in oldver:	
		newver.write(line)        

with open('./param/'+conn_weight+'.txt') as inf: # ./param/ contains all the .txt files with different parameter settings
    for line in inf:
        line_prepender('./compiled/smallnet_vb_contrast_'+conn_weight+'.py', line) # put the new file (file+parameters) into a folder called 
        															  # ./compiled, with the name of the file after the parameter
        															  # set's name, then .py as the format

with open('./compiled/smallnet_vb_contrast_'+conn_weight+'.py') as oldver, open('./smallnet_vb_contrast_'+conn_weight+'.py','w') as newver:
    for line in oldver:
        newver.write(line) 
##############################################################################################
...
for ls_condition in range(1,5):
	...
	addpath='./run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number
	if not os.path.exists(addpath): 
		os.makedirs(addpath)

	save_relay = './run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/relay'
	if not os.path.exists(save_relay): 
		os.makedirs(save_relay)
	for i in range (0, num_tc_cell):
		save_tc_cell = './run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/relay/tc_cell'+str(i)
		if not os.path.exists(save_tc_cell): 
			os.makedirs(save_tc_cell)

	save_reticular = './run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/reticular'
	if not os.path.exists(save_reticular): 
		os.makedirs(save_reticular)
	for i in range (0, num_re_cell):
		save_re_cell = './run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/reticular/re_cell'+str(i)
		if not os.path.exists(save_re_cell): 
			os.makedirs(save_re_cell)

tc_results_dat='/Users/jiemei/Desktop/TC_model/run_lgn_pgn/smallnet/'+conn_weight+'/tc_csr_results_dat/'
if not os.path.exists(tc_results_dat): 
	os.makedirs(tc_results_dat)	

re_results_dat='/Users/jiemei/Desktop/TC_model/run_lgn_pgn/smallnet/'+conn_weight+'/re_csr_results_dat/'
...
for ls_condition in range(1,5):	
	for trial_num in range(1,101):	
		...
		spiking3='"contrast/csr'+con_number+'/csr'+con_number+'_rgccell_3976/csr'+con_number+'_rgccell_3976_0'+str(trial_num)+'.dat"'
		spiking2='"contrast/csr'+con_number+'/csr'+con_number+'_rgccell_4188/csr'+con_number+'_rgccell_4188_0'+str(trial_num)+'.dat"'
		spiking1='"contrast/csr'+con_number+'/csr'+con_number+'_rgccell_1484/csr'+con_number+'_rgccell_1484_0'+str(trial_num)+'.dat"'
		spiking0='"contrast/csr'+con_number+'/csr'+con_number+'_rgccell_1272/csr'+con_number+'_rgccell_1272_0'+str(trial_num)+'.dat"'
		...
		dir_tc = './run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/relay/tc_cell' # cell 0...n
		dir_re = './run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/reticular/re_cell'
		...
		with open('/Users/jiemei/Desktop/TC_model/param/hoc_name.txt') as ref: # add parameters from the complete parameter file to the python script to run, then run it
			for lines in ref:
				print lines
				line_prepender('smallnet_vb_contrast_'+conn_weight+'.py', lines)
		
		os.system('python '+ 'smallnet_vb_contrast_'+conn_weight+'.py')

		os.remove('smallnet_vb_contrast_'+conn_weight+'.py')
...
for ls_condition in range(1,5):
	for i in range(0, num_tc_cell):
		...
		tc_results = '/run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/relay/tc_cell'
		os.chdir('/Users/jiemei/Desktop/TC_model'+tc_results+str(i))

		read_files = glob.glob("*.dat")

		#with open('tc_cell'+str(i)+'_csr'+con_number+'.dat', "wb") as outfile:
	...
	for i in range(0, num_re_cell):	
		...
		if ls_condition == 4:
			con_number = '100'			
		re_results = '/run_lgn_pgn/smallnet/'+conn_weight+'/contrast_'+con_number+'/reticular/re_cell'
	...
	for i in range(0, num_tc_cell):		
		...
		os.chdir('/Users/jiemei/Desktop/TC_model')
		tc_results = './run_lgn_pgn/smallnet/'+conn_weight+'/tc_csr_results'
		...
		plt.plot(tc_plot)
		plt.ylim(0, 100)
		os.chdir('/Users/jiemei/Desktop/TC_model/run_lgn_pgn/smallnet/'+conn_weight+'/tc_csr_results')
	...
	for i in range(0, num_re_cell):	
		...
		os.chdir('/Users/jiemei/Desktop/TC_model')
		re_results = './run_lgn_pgn/smallnet/'+conn_weight+'/re_csr_results'
		...
		plt.plot(re_plot)
		plt.ylim(0, 100)
		os.chdir('/Users/jiemei/Desktop/TC_model/run_lgn_pgn/smallnet/'+conn_weight+'/re_csr_results')
